...
void CentralWidget::initialize() {
    ...
    ui->pushButton_1->click(); // 显示第一个 widget
}
...
void CentralWidget::createRelatedWidget(QAbstractButton *button) {
    if (button == ui->pushButton_1) {
        ...
        QWidget *w = new QWidget();
        w->setStyleSheet("background: pink");
        buttonWidgetHash.insert(ui->pushButton_2, w);

        UiUtil::addWidgetIntoStackedWidget(w, ui->contentStackedWidget);
    } else if (button == ui->pushButton_3) {
        QWidget *w = new QWidget();
        w->setStyleSheet("background: blue");
        buttonWidgetHash.insert(ui->pushButton_3, w);

        UiUtil::addWidgetIntoStackedWidget(w, ui->contentStackedWidget);
    } else if (button == ui->pushButton_4) {
        QWidget *w = new QWidget();
        w->setStyleSheet("background: green");
        buttonWidgetHash.insert(ui->pushButton_4, w);

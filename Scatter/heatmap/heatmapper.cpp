 ...
 */
HeatMapper::HeatMapper(QImage *image, GradientPalette *palette, int radius, int opacity)
 ...
 * @param fileName 文件名
 */
void HeatMapper::save(const QString &fileName) {
    mainCanvas_->save(fileName);
}
...
void HeatMapper::colorize(int left, int top, int right, int bottom)
{
	...
	for (int i = left; i < right; ++i) {
		for (int j = top; j < bottom; ++j) {
            ...
            mainCanvas_->setPixel(i, j, qRgba(color.red(), color.green(), color.blue(), finalAlpha));
		}
	}
}
